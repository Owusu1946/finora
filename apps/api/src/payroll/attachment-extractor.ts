...
function canonicalHeader(value: unknown) {
  ...
  if (/^(destinationtype|accounttype|paymenttype|payouttype)$/.test(header))
    return 'destinationType';
  if (/^(phone|phonenumber|momonumber|account|accountnumber|destination)$/.test(header))
    return 'destination';
...
function rowsFromMatrix(
  ...
  return data.slice(0, MAX_ROWS).map((values, index) => {
    ...
    if (amount == null)
      rowIssues.push({
        code: 'amount_missing_or_invalid',
    ...
    if (!destination)
      rowIssues.push({
        ...
        message: 'A payout destination is missing.',
        rowId,
        blocking: true,
      });
    ...
    return PayrollImportRowSchema.parse({
      ...
      citations: [
        { sourceName, location: `${locationPrefix}, row ${index + (hasHeader ? 2 : 1)}` },
      ],
      issues: rowIssues,
    });
  });
  ...
  if (!delimiter) {
    const naturalRows = lines.map((line) => {
      const amountMatch = line.match(
        /(?:GHS|USD|EUR|GBP|NGN|KES|ZAR|\$|€|£)?\s*([0-9][0-9,]*(?:\.\d{1,2})?)/i,
      );
...
export async function extractPayrollRows(input: {
  ...
  if (input.contentType.startsWith('image/') && input.extractImage) {
    const matrix = await input.extractImage({
  ...
  if (input.contentType.startsWith('image/'))
    throw new Error('payroll_image_extraction_unavailable');
