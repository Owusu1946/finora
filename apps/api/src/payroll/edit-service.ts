...
import { payrollEditProposals, payrollImportRows, payrollImports } from '../db/schema';
...
export function applyProposedChanges(rows: PayrollImportRow[], rawChanges: unknown[]) {
  ...
  const resolved = changes.map((change) => {
    ...
    const after = validatePayrollRow(
      PayrollImportRowSchema.parse({ ...before, ...patch, rowId: before.rowId }),
    );
...
export async function listPayrollImportsForChat(
  databaseUrl: string,
  userId: string,
  input: { query?: string; importId?: string; limit: number },
) {
  const db = createDb(databaseUrl);
  const imports = await db
    .select()
    .from(payrollImports)
    .where(and(eq(payrollImports.clerkUserId, userId), isNull(payrollImports.deletedAt)))
    .orderBy(desc(payrollImports.updatedAt))
    .limit(50);
  const result = [];
  const normalizedQuery = input.query?.trim().toLowerCase();
  for (const item of imports.filter(
    (candidate) => !input.importId || candidate.id === input.importId,
  )) {
    const rawRows = await db
      .select({ payload: payrollImportRows.payload })
      .from(payrollImportRows)
    ...
    const matchingRows = normalizedQuery
      ? allRows.filter((row) =>
          [row.employeeName, row.employeeId].some((value) =>
    ...
    result.push({
      ...
      status: item.status,
      rows: matchingRows.slice(0, input.limit),
    });
...
export async function proposePayrollChanges(databaseUrl: string, userId: string, input: unknown) {
  ...
  const afterState = {
    ...snapshot(item, afterRows),
    status: afterSummary.blockingIssues.length ? 'blocked' : 'ready',
  };
  const [proposal] = await db
    .insert(payrollEditProposals)
    .values({
      clerkUserId: userId,
      importId: item.id,
    ...
    .returning({ id: payrollEditProposals.id, expiresAt: payrollEditProposals.expiresAt });
...
export async function applyPayrollChanges(databaseUrl: string, userId: string, proposalId: string) {
  ...
  const [proposal] = await db
    .select()
    .from(payrollEditProposals)
    .where(
      and(eq(payrollEditProposals.id, proposalId), eq(payrollEditProposals.clerkUserId, userId)),
    )
    .limit(1);
  if (!proposal) throw new PayrollEditError('payroll_proposal_not_found', 404);
  if (proposal.status !== 'pending')
    throw new PayrollEditError('payroll_proposal_already_resolved', 409);
  if (proposal.expiresAt.getTime() <= Date.now()) {
    await db
      .update(payrollEditProposals)
      .set({ status: 'expired' })
      .where(eq(payrollEditProposals.id, proposal.id));
    throw new PayrollEditError('payroll_proposal_expired', 409);
  }
  const [item] = await db
    .select()
    .from(payrollImports)
    .where(
      and(
        eq(payrollImports.id, proposal.importId),
        eq(payrollImports.clerkUserId, userId),
        isNull(payrollImports.deletedAt),
      ),
    )
    .limit(1);
  if (!item || item.version !== proposal.baseVersion) {
    await db
      .update(payrollEditProposals)
      .set({ status: 'stale' })
      .where(eq(payrollEditProposals.id, proposal.id));
    throw new PayrollEditError('payroll_proposal_stale', 409);
  }
  const rawRows = await db
    .select()
    .from(payrollImportRows)
  ...
  const current = new Map(
    rawRows.map((row) => [row.rowId, PayrollImportRowSchema.parse(row.payload)]),
  );
  for (const change of proposal.changes as Array<{
    rowId: string;
    operation: 'update' | 'delete';
    after?: PayrollImportRow | null;
  }>) {
  ...
  const result = await db.$client.query(
    ...
    [
      ...
      summary.currency,
      JSON.stringify(summary.blockingIssues),
...
export async function cancelPayrollProposal(
  databaseUrl: string,
  userId: string,
  proposalId: string,
) {
  ...
  const [cancelled] = await db
    ...
    .where(
      and(
        eq(payrollEditProposals.id, proposalId),
        eq(payrollEditProposals.clerkUserId, userId),
        eq(payrollEditProposals.status, 'pending'),
      ),
    )
    .returning({ id: payrollEditProposals.id });
