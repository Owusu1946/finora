import type { Currency, SettlementMethod } from './enums';

/** Field keys collected in the send destination step per rail. */
...
export type CorridorCountry = {
  ...
  code: string;
  name: string;
  /** ISO 3166-1 alpha-3 (WeWire beneficiary country) */
  alpha3: string;
  /** Default payout currency for this corridor */
  currency: Currency;
  rails: SettlementMethod[];
  /** Required destination fields keyed by settlement method */
  fields: Partial<Record<SettlementMethod, CorridorFieldKey[]>>;
};

/**
 * Curated representative corridors (expandable).
 * Country reach for agents / UI — not a live WeWire discovery API.
 */
...
/** Global crypto corridor (not tied to a single country). */
export const CRYPTO_CORRIDOR: {
  ...
  rails: ['CRYPTO'],
  fields: ['cryptoAddress', 'blockchain', 'accountName'],
...
/** Rough mock FX table for on-device / API preview (not live WeWire rates). */
